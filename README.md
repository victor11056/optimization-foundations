...
print(a.grad == d / a) 
5. 学習のポイント逆向きに微分する理由: 出力（誤差）から遡ることで、数百万のパラメータに対する勾配をたった1回のパスで効率よく計算できるため。計算グラフの切断 (detach): 強化学習やGANなど、特定の変数を更新したくない（定数として扱いたい）場合に非常に強力なツールとなる。次にやっておくと良いことこの内容をGitHubにアップロードする際、「自分の言葉で一言コメント」（例：zero_() を忘れて1時間悩んだ、など）を添えると、後で見返したときに「生きた知識」になりますよ！このMarkdownの中で、もっと詳しく説明を加えたい部分はありますか？ もしあれば追記します！
